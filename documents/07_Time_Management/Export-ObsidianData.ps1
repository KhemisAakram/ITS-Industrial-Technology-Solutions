...
# Write JSON
$jsonPath = Join-Path $outputDir "obsidian_data.json"
$output | ConvertTo-Json -Depth 5 | Set-Content -Path $jsonPath -Encoding UTF8

# Write JS (for file:// protocol - browser can't fetch local JSON)
$jsPath = Join-Path $outputDir "obsidian_data.js"
$jsContent = "// Auto-generated by Export-ObsidianData.ps1 - do not edit`nwindow.obsidianData = " + ($output | ConvertTo-Json -Depth 5) + ";"
...
Write-Host "Output: $jsonPath + $jsPath"
